...
def get_ec2_instances(search='*'):
    ...
    # If the function is called directly (e.g. from the command line)
    if __name__ == '__main__':
        print(table)
        print(f"\nTotal EBS size across all servers: {total_ebs_size} GB")
        return

    # If the function is called from AWS Lambda
    return {
        'body': str(table),
        'totalEBSsize': f"Total EBS size across all servers: {total_ebs_size} GB"
    }
